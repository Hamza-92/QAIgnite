                ...
                <tbody>
                    @forelse ($requirements as $requirement)
                        <tr wire:key='{{ $requirement->id }}' class="hover:bg-gray-100 dark:hover:bg-gray-800">
                            ...
                            <template x-if="requirement_title_col">
                                <td class="px-4 py-3" title="{{ $requirement->requirement_title }}">{{ $requirement->requirement_title }}</td>
                            </template>
                            <template x-if="requirement_summary_col">
                                <td class="px-4 py-3" title="{{ $requirement->requirement_summary }}">{{ Str::limit($requirement->requirement_summary, 30, '...') }}</td>
                            </template>
                            <template x-if="status_col">
                                <td class="px-4 py-3" title="{{$requirement->status}}">{{ $requirement->status ?? '' }}</td>
                            </template>
                            <template x-if="created_date_col">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap">
                                <div class="flex items-center justify-center space-x-1">
                                    ...
                                    <button type="button" wire:loading.attr="disabled"
                                        wire:target="delete({{ $requirement->id }})" x-data="{
                                            confirmDelete() {
                                                Swal.fire({
                                                    title: 'Are you sure?',
                                        ...
                                        title="Delete">
                                        <i wire:loading.remove wire:target="delete({{ $requirement->id }})"
                                            class="fa-solid fa-trash-can"></i>
                                        <span wire:loading wire:target="delete({{ $requirement->id }})"
                                            class="ml-1">
                                            <i class="fa-solid fa-spinner animate-spin"></i>
                                        </span>
                                    </button>
                    ...
                    <div class="relative">
                        <select wire:model.live='build_id' wire:change='updateModulesList' name="build_id"
                            ...
                            @forelse ($builds as $build)
                                <option wire:key='{{ $build->id }}' value="{{ $build->id }}">
                                    {{ $build->name }}</option>
                            @empty
                            @endforelse
                        </select>
                        ...
                        <select wire:model.live='module_id' name="module_id" id="module_id"
                            ...
                            @isset($modules)
                                @foreach ($modules as $module)
                                    <option class="text-white" wire:key='{{ $module->id }}'
