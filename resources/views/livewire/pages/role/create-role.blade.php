<div>
    <div class="px-8 py-4 space-y-4 border-b dark:border-gray-700">
        ...
        <h2 class="text-lg font-medium">Create New Role</h2>
    </div>

    <div class="p-8 grid grid-cols-2 w-full">
        <div class="border-r dark:border-gray-700 pr-8">
            <form wire:submit.prevent='saveRole'>
                <div class="flex flex-col gap-1">
                    <label for="name">Name <span class="text-sm text-red-500">*</span></label>
                    <input wire:model='name' type="text" name="name" id="name" required autocomplete="Role" class="rounded-md border-gray-200 dark:bg-gray-700 dark:border-gray-700 text-sm">
                    @error('name')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="flex flex-col gap-1 mt-4">
                    <label for="description">Description <span class="text-sm text-red-500">*</span></label>
                    <textarea wire:model='description' type="text" name="description" id="description" required rows="5" class="rounded-md border-gray-200 dark:bg-gray-700 dark:border-gray-700 text-sm"></textarea>
                    @error('description')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <button type="submit" class="bg-blue-500 hover:bg-blue-600 dark:bg-blue-800 dark:hover:bg-blue-900 text-white px-4 py-2 rounded-md mt-4">Save</button>
            </form>
        </div>
        <div class="flex flex-col gap-4 pl-8">
            {{-- <label for="allPermissions"><input wire:model='allPermissions' type="checkbox" name="allPermissions" id="allPermissions">  Grant all Permissions</label> --}}
            <p>Select all permissions that apply</p>

            {{-- Project Management --}}
            <div x-data="{ open: false }" class="">
                <div @click="open = !open" class="flex justify-between items-center px-4 py-2 bg-blue-500 hover:bg-blue-600 dark:bg-blue-800 dark:hover:bg-blue-900 cursor-pointer">
