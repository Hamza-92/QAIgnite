<div>
    <div class="px-8 py-4 flex items-center justify-between gap-4 border-b dark:border-gray-700">
        <h2 class="text-lg font-medium">Edit Test Cycle</h2>
    ...
    <div class="p-8 space-y-6">
        <form wire:submit.prevent='save' class="space-y-6">
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                ...
                {{-- Assigne To --}}
                <div class="flex flex-col gap-1 w-full">
                    <label class="flex items-center gap-2">Assign To <i class="fa-solid fa-circle-info"
                            title="Assign the execution cycle to your team members."></i></label>
                    <div x-data='{open_model : false}' class="relative w-full" @click.outside="open_model = false"
                        @close.stop="open_model = false">
                            ...
                            <div class="w-full flex flex-wrap items-center justify-start gap-2">
                                @forelse ($form_selected_users as $user)
                                    <div wire:key='{{ $user['id'] }}'
                                        class="flex gap-2 bg-gray-100 dark:bg-gray-800 py-1 px-2 rounded-sm">
                                        <span class="border-r dark:border-gray-700">{{ $user['username'] }}</span>
                                        <button type="button" wire:click='removeUser({{ $user['id'] }})'>
                                            <i wire:loading.remove wire:target='removeUser({{ $user['id'] }})'
                                                class="fa-solid fa-xmark"></i>
                                            <i wire:loading wire:target='removeUser({{ $user['id'] }})'
                ...
                <div wire:ignore x-data x-init="flatpickr($refs.startDate, {
                    ...
                    dateFormat: 'Y-m-d',
                    minDate: 'today',
                    defaultDate: '{{ $test_cycle->start_date }}',
                ...
                <div wire:ignore x-data="{ endPicker: null, minDate: @entangle('start_date') }" x-init="endPicker = flatpickr($refs.endDate, {
                    ...
                    dateFormat: 'Y-m-d',
                    minDate: minDate,
                    defaultDate: '{{ $test_cycle->end_date }}',
                    onChange: function(selectedDates, dateStr) {
                ...
                <div>
                    <label class="flex gap-2 items-center">Cycle Visibility <i class="fa-solid fa-circle-info"
                            title="By checking this, only the tester role will be able to view and execute the test cases assigned in that cycle."></i></label>
                ...
                {{-- Description --}}
                <x-textarea label='Description' class="col-span-full" model='description'
                    rows='7'></x-textarea>
            </div>
            ...
            <div class="py-16 flex items-center justify-center gap-4">
                <button type="submit" wire:loading.attr='disabled'
                    ...
                    <i class="fa-solid fa-plus"></i> Save <i wire:loading wire:target='save'
                        class="fa-solid fa-spinner fa-spin"></i>
                </button>
