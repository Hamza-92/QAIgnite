...
class AttachDefect extends Component
{
    public $test_cycle;
    public $test_case;
    ...
    public function mount($test_cycle_id, $test_case_id, $test_case_execution_id)
    {
        $this->test_cycle = TestCycle::where('id', $test_cycle_id)
            ->where('project_id', auth()->user()->default_project)
            ->first();

        $this->test_case = TestCase::where('id', $test_case_id)
            ->where('tc_project_id', auth()->user()->default_project)
            ->first();

        $this->test_case_execution = TestCaseExecution::where('id', $test_case_execution_id)
            ->where('test_cycle_id', $test_cycle_id)
            ->where('test_case_id', $test_case_id)
            ->first();

        if ($this->test_cycle && $this->test_case && $this->test_case_execution) {
            ...
            $this->defect_steps_to_reproduce = $this->test_case->tc_detailed_steps;
            $this->defect_actual_result = null;
            $this->defect_expected_result = $this->test_case->tc_expected_result;
            $this->defect_project_id = auth()->user()->default_project;
            $this->defect_build_id = $this->test_case->tc_build_id;
            $this->defect_module_id = $this->test_case->tc_module_id;
            $this->defect_requirement_id = $this->test_case->tc_requirement_id;
            $this->defect_test_scenario_id = $this->test_case->tc_test_scenario_id;
            $this->defect_test_case_id = $this->test_case->id;
            $this->defect_created_by = auth()->user()->id;
            $this->defect_assigned_to = null;
            $this->comment = null;

            $this->form_selected_build_name = Build::where('id', $this->defect_build_id)->value('name') ?? null;
            $this->form_selected_module_name = Module::where('id', $this->defect_module_id)->value('module_name') ?? null;
            $this->form_selected_requirement_name = Requirement::where('id', $this->defect_requirement_id)->value('requirement_title') ?? null;
            $this->form_selected_test_scenario_name = TestScenario::where('id', $this->defect_test_scenario_id)->value('ts_name') ?? null;
            $this->form_selected_test_case_name = TestCase::where('id', $this->defect_test_case_id)->value('tc_name') ?? null;
            $this->loadFormBuilds();
    ...
    public function save() {
        ...
        $this->test_case_execution->update([
            'defect_id' => $defect->id
        ]);

        Toaster::success('Defect created successfully.');

        $this->redirect(route('test-case-execution.execute', [$this->test_cycle->id, $this->test_case->id]), true);
    }
    public function render()
    {
        return view('livewire.defect.attach-defect');
    }
}
